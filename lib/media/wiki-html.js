...
Media({
	...
	serialize: function(object, request, response){
		var pageName = escapeHTML(decodeURIComponent(request.pathInfo.substring(1)));
		var action;
		...
		return {
			forEach:function(write){
				...
				write('<script>// Configure RequireJS\nrequire = {baseUrl: "lib/"};</script>\n');
				write('<script src="http://requirejs.org/docs/release/0.11.0/minified/transportD-require.js"></script>\n');
